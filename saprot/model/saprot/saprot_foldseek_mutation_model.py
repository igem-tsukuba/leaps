...
class SaprotFoldseekMutationModel(SaprotBaseModel):
    ...
    def get_struc_seq(self, structure_content, structure_type, plddt):
        ...
        tmp_pdb_path = (
            f"EsmFoldseekMutationModel_{self.global_rank}_{rank}.{structure_type}"
        )
        ...
        if self.mask_rate is not None:
            ...
            indices = np.random.choice(
                len(struc_seq), int(len(struc_seq) * self.mask_rate), replace=False
            )
            np_seq = np.array(list(struc_seq))
            np_seq[indices] = "#"
            struc_seq = "".join(np_seq)
    ...
    def forward(
        self, wild_type, seqs, mut_info, structure_content, structure_type, plddt
    ):
        device = self.device

        if getattr(self, "struc_seq", None) is None:
            self.struc_seq = self.get_struc_seq(
                structure_content, structure_type, plddt
            )
